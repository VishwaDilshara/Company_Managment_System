...
function AddVehicle() {
  ...
  const handleSubmit = async () => {
    ...
    if (checkEmptyFields(post1) === true) {
      setValidated(true);
    } else {
      setValidated(true);
      // AXIOS
      await axios
        .post("/createdistributer", post1)
        .then((res) => {
          console.log(res);

          navigate("/DistributeManager/createvehicle/VehicleList"); //after create a post move to the distribute page
        })
